...
<template>
  <div class="gol-container">
    <div class="header">
      ...
      <div class="controls">
        <button class="control-btn play" @click="togglePlay" :class="{ 'is-running': isRunning }">
          {{ isRunning ? '⏸ Pause' : '▶ Play' }}
        </button>
        <button class="control-btn step" @click="step" :disabled="isRunning">⏭ Step</button>
        <button class="control-btn" @click="randomize">🎲 Random</button>
        <button class="control-btn" @click="clear">🗑️ Clear</button>
        
        <div class="speed-control">
          <label>Speed</label>
          <input type="range" v-model.number="speed" min="1" max="60" />
        </div>
      </div>
    </div>
