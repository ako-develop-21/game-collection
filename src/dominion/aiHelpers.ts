...
import type { GameState, Card, PromptType, PlayerState, ActionHelpers } from './types'

// --------------------------------------------------------------------------------
// ゲーム進行度
// --------------------------------------------------------------------------------
export const isEarlyGame = (state: GameState) => (state.supply['province'] || 0) > 6;
export const isMidGame = (state: GameState) => {
  const prov = state.supply['province'] || 0;
  return prov > 2 && prov <= 6;
};
export const isLateGame = (state: GameState) => (state.supply['province'] || 0) <= 2;
...
export const needsCoins = (state: GameState, playerIndex: number) => {
  ...
  const totalCoins = allCards.reduce((sum, c) => {
    ...
    else if (typeof c.treasure === 'function') return sum + c.treasure(state, playerIndex);
    return sum;
  }, 0);
  return totalCoins < 5;
};
 ...
 * @returns 
 */
const getAIVictoryCardValue = (card: Card, state: GameState, playerIndex: number, promptType: PromptType) => {
  let point = 0;
  if (typeof card.points === 'number') point = card.points;
  else if (typeof card.points === 'function') point = card.points(state, playerIndex);
 ...
 * @returns 
 */
export const getAICardValueInHand = (card: Card, state: GameState, playerIndex: number, promptType: PromptType) => {
  if (card.ai?.getValue) {
  ...
  if (card.type.includes('Action')) {
    ...
    // サプライにアタックカードがある場合
    const supplyCards = Object.keys(state.supply).map(cardId => ALL_CARDS[cardId]!);
...
// 購入時のカード評価
// --------------------------------------------------------------------------------
export const getAIPurchaseWeight = (cardId: string, coins: number, state: GameState, playerIndex: number, helpers: ActionHelpers,) => {
  ...
  const allCards = [...player.hand, ...player.deck, ...player.discard, ...player.inPlay];
  const totalTreasureCoins = allCards.reduce((sum, c) => sum + helpers.getCardTreasure(c.id, player.id), 0);
  ...
  const deckSize = player.deck.length + player.hand.length + player.discard.length;

  // 1. Three-pile ending monitoring
  const emptyPiles = Object.entries(supply).filter(([id, count]) => count === 0 && id !== 'curse').length;
  const nearEmptyPiles = Object.entries(supply).filter(([id, count]) => count > 0 && count <= 2 && id !== 'curse').length;
  ...
  const opponentPoints = Math.max(...playerPoints.filter((_, i) => i !== aiIndex), 0);
  ...
  if (emptyPiles >= 2 || (emptyPiles === 1 && nearEmptyPiles >= 2)) {
    if (isWinning) {
      // Boost cards that end the game
        // weight += 800;
        if (cardId !== 'estate' && cardId !== 'duchy' && cardId !== 'province') {
  ...
  if (card.type.includes('Action')) {
    ...
    const terminalActions = allCards.filter(c => c.type.includes('Action') && !(c.effects?.action)).length;
    const actionGivers = allCards.filter(c => (c.effects?.action || 0) >= 1).length;
