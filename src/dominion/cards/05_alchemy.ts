...
export const ALCHEMY_CARDS: Record<string, Card> = {
    ...
    apothecary: {
        ...
        costPotion: 1,
        description:
            "+1 カード, +1 アクション / あなたの山札の上から4枚のカードを公開する。銅貨とポーションを公開した場合、それをあなたの手札へ加える。残りのカードは好きな順番であなたの山札の上に置く。",
    ...
    familiar: {
        ...
        costPotion: 1,
        description:
            "+1 カード, +1 アクション / 他のプレイヤーは呪いカードを1枚獲得する。",
                ...
                onConfirm: (indices: number[]) => {
                    ...
                    if (card) {
                        ...
                        if (card.type.includes("Action"))
                            helpers.gainCard(playerIndex, "duchy");
                        if (card.type.includes("Treasure"))
                            helpers.gainCard(playerIndex, "transmute");
    ...
    alchemist: {
        ...
        onCleanup: (state, playerIndex, helpers, _, cardIndex) => {
            const player = state.players[playerIndex]!;
                ...
                helpers.setPrompt({
                    ...
                    onConfirm: (choice: string) => {
                        if (choice === "はい" || choice === "YES") {
                            ...
                            helpers.addLog(
                                `${player.name} はポーションをプレイしているため、錬金術師を山札の一番上に置きました。`,
                            );
                        } else {
    ...
    herbalist: {
        ...
        onCleanup: (state, playerIndex, helpers, _, cardIndex) => {
            ...
            const treasuresInPlay = player.inPlay.filter((c) =>
                c.type.includes("Treasure"),
            );
            if (treasuresInPlay.length > 0) {
                const options = Array.from(
                    new Set(treasuresInPlay.map((t) => t.id)),
                );
                const getJpName = (id: string) =>
                    player.inPlay.find((c) => c.id === id)?.jpName || id;
                ...
                helpers.setPrompt({
                    ...
                    max: 1,
                    message:
                        "薬草商の効果：山札の一番上に置く財宝カードを選んでください。",
                    options: optionNamesLocalized,
                    onConfirm: (choice: string) => {
                        player.discard.push(herbalist);
                        if (choice !== "スキップ") {
                            const targetId =
                                options[optionNamesLocalized.indexOf(choice)];
                            const targetIndex = player.inPlay.findIndex(
                                (c) =>
                                    c.id === targetId &&
                                    c.type.includes("Treasure"),
                            );
                            if (targetIndex !== -1) {
                                const treasure = player.inPlay.splice(
                                    targetIndex,
                                    1,
                                )[0]!;
                                player.deck.push(treasure);
                                helpers.addLog(
                                    `${player.name} は薬草商の効果で ${treasure.jpName} を山札の一番上に置きました。`,
                                );
                            }
                        }
                        helpers.clearPrompt();
    ...
    apprentice: {
        ...
        cost: 5,
        description:
            "+1 アクション / 手札のカード1枚を廃棄する。そのカードのコスト1につき+1カード、コストに🧪が含まれていればさらに+2カード。",
            ...
            helpers.setPrompt({
                ...
                onConfirm: (indices: number[]) => {
                    ...
                    if (card) {
                        ...
                        const drawCount =
                            trashedCost + (trashedPotion > 0 ? 2 : 0);
    ...
    university: {
        ...
        costPotion: 1,
        description:
            "+2 アクション / コスト5までのアクションカード1枚を獲得してもよい。",
        effects: { action: 2 },
        action: (_state, playerIndex, helpers) => {
            helpers.setPrompt({
                ...
                allowedTypes: ["Action"],
                message:
                    "大学：コスト5までのアクションカードを獲得できます（キャンセル可能）",
    ...
    golem: {
        ...
        costPotion: 1,
        description:
            "山札からゴーレム以外のアクションカード2枚が出るまで公開する。それ以外を捨て札にし、2枚を好きな順番でプレイする。",
        action: (state, playerIndex, helpers) => {
            const player = state.players[playerIndex]!;
            ...
            if (revealedCards.length > 0) {
                ...
                helpers.addLog(
                    `${player.name} はゴーレムの効果で見つからなかったカードを捨て札にしました。`,
                );
            }

            if (foundActions.length === 0) {
                helpers.addLog(
                    "ゴーレムの効果：アクションカードが見つかりませんでした。",
                );
                return;
            }
            ...
            const executeSequentially = (cardsToPlay: Card[]) => {
                ...
                helpers.addLog(
                    `${player.name} はゴーレムの効果で ${currentCard.jpName} をプレイします。`,
                );
                ...
                if (currentCard.effects) {
                    if (currentCard.effects.draw)
                        helpers.drawCards(
                            playerIndex,
                            currentCard.effects.draw,
                        );
                    if (currentCard.effects.action)
                        player.actions += currentCard.effects.action;
                    if (currentCard.effects.buy)
                        player.buys += currentCard.effects.buy;
                    if (currentCard.effects.coin)
                        player.coins += currentCard.effects.coin;
                }
            ...
            player.turnFlags = {
                ...player.turnFlags,
                executingCount: (player.turnFlags?.executingCount || 0) + 1,
            };
            ...
            if (foundActions.length === 1) {
                ...
                if (player.name === "You") {
                    helpers.setPrompt({
                        ...
                        onConfirm: (choice: string) => {
                            const firstIdx = foundActions.findIndex(
                                (c) => c.jpName === choice,
                            );
                            const ordered = [
                                foundActions[firstIdx]!,
                                foundActions[1 - firstIdx]!,
                            ];
                            helpers.clearPrompt();
    ...
    scrying_pool: {
        ...
        costPotion: 1,
        description:
            "+1 アクション / 各プレイヤーは山札の一番上を公開し、あなたがそれを捨て札にするか戻すか選ぶ。その後、アクション以外が出るまで公開しすべて手札に加える。",
                    ...
                    if (playerIndex === targetIdx) {
                        ...
                        if (currentPlayer.name.startsWith("AI")) {
                            if (val < 20) {
                                ...
                                helpers.addLog(
                                    `${victim.name} は自分の ${card.jpName} を捨て札にしました。`,
                                );
                            } else {
                                victim.deck.push(card);
                                helpers.addLog(
                                    `${victim.name} は自分の ${card.jpName} を山札に戻しました。`,
                                );
                            }
                            next();
                        } else {
                        ...
                        if (currentPlayer.name.startsWith("AI")) {
                            ...
                            if (val >= 20) {
                                ...
                                victim.discard.push(card);
                                helpers.addLog(
                                    `${currentPlayer.name} は ${victim.name} の ${card.jpName} を捨て札にさせました。`,
                                );
                            } else {
                                victim.deck.push(card);
                                helpers.addLog(
                                    `${currentPlayer.name} は ${victim.name} の ${card.jpName} を山札に戻させました。`,
                                );
                            }
                            next();
                        } else {
                        ...
                        const revealNext = () => {
                            if (currentPlayer.deck.length === 0) {
                                if (currentPlayer.discard.length === 0) {
                                    ...
                                    if (revealedCards.length > 0) {
                                        ...
                                        helpers.addLog(
                                            `${currentPlayer.name} は ${revealedCards.map((c) => c.jpName).join("、")} を手札に加えました。`,
                                        );
                                    }
                                    return;
                                }
                                ...
                                helpers.addLog(
                                    `${currentPlayer.name} は捨て札をシャッフルしました。`,
                                );
                            }
                            ...
                            if (card) {
                                if (card.type.includes("Action")) {
                                    ...
                                    helpers.addLog(
                                        `${currentPlayer.name} は ${card.jpName} が出るまで公開しました。`,
                                    );
                                    if (revealedCards.length > 0) {
                                        currentPlayer.hand.push(
                                            ...revealedCards,
