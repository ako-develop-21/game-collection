...
export const INTRIGUE_CARDS: Record<string, Card> = {
  duke: {
    ...
    points: (state, playerIndex) => {
      ...
      const totalDuchies = [...player.hand, ...player.deck, ...player.discard, ...player.inPlay].filter(c => c.id === 'duchy').length;
      return totalDuchies;
    }
  },
  shanty_town: {
    ...
    type: ['Action'],
    cost: 3,
    description: '+2 アクション / 手札を公開する。手札にアクションカードが1枚もない場合、+2 カード。',
    effects: { action: 2 },
    action: (state, playerIndex, helpers) => {
      const player = state.players[playerIndex]!;
      const cardNames = player.hand.map(c => c.jpName).join('、');
      helpers.addLog(`${player.name} は貧民街の効果で手札を公開しました（${cardNames || '手札なし'}）。`);
      
      const hasAction = player.hand.some(c => c.type.includes('Action'));
      if (!hasAction) {
  ...
  conspirator: {
    ...
    type: ['Action'],
    cost: 4,
    description: '+2 コイン / これがこのターンで使用する3枚目以上のアクションカードである場合（このカードも数える）、+1 カード、+1 アクション。',
    effects: { coin: 2 },
    action: (state, playerIndex, helpers) => {
      const player = state.players[playerIndex]!;
      const actionCount = player.inPlay.filter(c => c.type.includes('Action')).length;
      if (actionCount >= 3) {
        helpers.drawCards(playerIndex, 1);
        player.actions += 1;
        helpers.addLog(`${player.name} は共謀者の条件を満たし、+1 カード、+1 アクションを得ました。`);
      }
    }
  },
  mill: {
    ...
    type: ['Action', 'Victory'],
    cost: 4,
    description: '+1 カード, +1 アクション / 手札を2枚捨ててもよい。そうした場合、+2 コイン。 / 1 勝利点',
    effects: { draw: 1, action: 1 },
    points: () => 1,
    ai: {
      getChoicePriorities: (state, playerIndex) => {
        const player = state.players[playerIndex]!;
        const junk = player.hand.filter(c => getAICardValueInHand(c, state, playerIndex, 'discard') < 20);
    ...
    action: (state, playerIndex, helpers) => {
      ...
      if (player.hand.length >= 2) {
        helpers.setPrompt({
          ...
          min: 1,
          max: 1,
          message: '製粉所：手札を2枚捨てますか？（2枚ちょうど捨てると+2コイン）',
          ...
          onConfirm: (choice: string) => {
            if (choice.startsWith('YES')) {
              helpers.setPrompt({
                ...
                min: 2,
                max: 2,
                message: '製粉所：捨てる手札を2枚選択してください',
                onConfirm: (indices: number[]) => {
                  player.coins += 2;
                  helpers.addLog(`${player.name} は製粉所の効果で2枚捨て、2コインを得ました。`);
        ...
        helpers.setPrompt({
          ...
          min: 1,
          max: 1,
          message: '製粉所：手札を1枚捨てますか？（追加コインは得られませんが、地下貯蔵庫などのトリガーになります）',
  ...
  steward: {
    ...
    type: ['Action'],
    cost: 3,
    description: '+2 カード、または +2 コイン、または 手札から2枚を廃棄する。',
    ai: {
      getChoicePriorities: (state, playerIndex) => {
        const player = state.players[playerIndex]!;
        const badCards = player.hand.filter(c => getAICardValueInHand(c, state, playerIndex, 'trash') < 20);
      ...
      helpers.setPrompt({
        ...
        onConfirm: (choice: string) => {
          if (choice === '+2 カード') {
            ...
            helpers.addLog(`${state.players[playerIndex]!.name} は+2 コインを得ました。`);
          } else if (choice === '2枚廃棄') {
            helpers.setPrompt({
              type: 'trash',
              targetPlayerIndex: playerIndex,
              ...
              min: 2,
              max: 2,
              message: '執事：廃棄するカードを2枚選択してください',
              onConfirm: (indices: number[]) => {
                const sorted = [...indices].sort((a,b)=>b-a);
                sorted.forEach(idx => helpers.trashCard(playerIndex, idx));
                helpers.clearPrompt();
              }
            });
  ...
  nobles: {
    ...
    cost: 6,
    points: 2,
    description: '2 勝利点 / 次のうちいずれか1つを選択する：(＋3 カード); (＋2 アクション)。',
    ai: {
      getChoicePriorities: (state, playerIndex) => {
        const player = state.players[playerIndex]!;
      ...
      helpers.setPrompt({
        ...
        onConfirm: (choice: string) => {
          if (choice === '+3 カード') {
            ...
            helpers.addLog(`${state.players[playerIndex]!.name} は+2 アクションを得ました。`);
          }
          helpers.clearPrompt();
        }
      });
  ...
  baron: {
    ...
    type: ['Action'],
    cost: 4,
    description: '+1 購入 / 手札から屋敷1枚を捨ててもよい。そうした場合、+4 コイン。しなければ、屋敷1枚を獲得する。',
    effects: { buy: 1 },
    ai: {
      getChoicePriorities: (state, playerIndex) => {
    ...
    action: (state, playerIndex, helpers) => {
      ...
      if (estateIdx !== -1) {
        helpers.setPrompt({
          ...
          options: ['YES (屋敷を捨てて+4コイン)', 'NO (屋敷を獲得する)'],
          onConfirm: (choice: string) => {
            if (choice.startsWith('YES')) {
              const idx = player.hand.findIndex(c => c.id === 'estate');
              if (idx !== -1) {
                const card = player.hand.splice(idx, 1)[0]!;
                player.discard.push(card);
                player.coins += 4;
                helpers.addLog(`${player.name} は屋敷を捨てて+4 コインを得ました。`);
              }
            } else {
  ...
  trading_post: {
    ...
    type: ['Action'],
    cost: 5,
    description: '手札からカード2枚を廃棄する。そうした場合、銀貨1枚を獲得し、手札に加える。',
    action: (state, playerIndex, helpers) => {
      const player = state.players[playerIndex]!;
  ...
  upgrade: {
    ...
    type: ['Action'],
    cost: 5,
    description: '+1 カード, +1 アクション / 手札からカード1枚を廃棄する。廃棄したカードよりコストがちょうど1高いカード1枚を獲得する。',
    effects: { draw: 1, action: 1 },
    action: (state, playerIndex, helpers) => {
      const player = state.players[playerIndex]!;
      if (player.hand.length === 0) return;
      ...
      helpers.setPrompt({
        ...
        onConfirm: (indices: number[]) => {
          ...
          if (card) {
            ...
            const canGain = Object.entries(state.supply).some(([id, count]) => {
              const currentCost = helpers.getCardCost(id, playerIndex);
              const c = helpers.getCard(id);
              return currentCost === targetCost && (c?.costPotion || 0) === trashedPotion && count > 0;
            });

            if (!canGain) {
              helpers.addLog(`サプライに現在のコストが ${targetCost}${trashedPotion > 0 ? '+🧪' : ''} のカードがないため、獲得は行われません。`);
              helpers.clearPrompt();
              return;
            }
            ...
            helpers.setPrompt({
              ...
              onConfirm: (gainId: string) => {
                const currentGainCost = helpers.getCardCost(gainId, playerIndex);
                const targetCard = helpers.getCard(gainId);
                if (currentGainCost === targetCost && (targetCard?.costPotion || 0) === trashedPotion) {
  ...
  patrol: {
    ...
    type: ['Action'],
    cost: 5,
    description: '+3 カード / 山札の上から4枚を公開する。その中の勝利点カードと呪いカードをすべて手札に加え、残りを好きな順で山札の上に戻す。',
    effects: { draw: 3 },
    action: (state, playerIndex, helpers) => {
      const player = state.players[playerIndex]!;
      ...
      if (toHand.length > 0) {
        helpers.addLog(`${player.name} は公開された中から ${toHand.map(c => c.jpName).join(', ')} を手札に加えました。`);
      }
      ...
      if (backToDeck.length > 0) {
        if (backToDeck.length === 1) {
          ...
          helpers.setPrompt({
            ...
            message: 'パトロール：山札に戻す順番を（上から順に）選択してください',
            onConfirm: (indices: number[]) => {
  ...
  torturer: {
    ...
    type: ['Action', 'Attack'],
    cost: 5,
    description: '+3 カード / 他のプレイヤーは全員、次のいずれか1つを選択する。「手札からカード2枚を捨てる」「呪いカード1枚を獲得し、手札に加える」。',
    ...
    action: (state, playerIndex, helpers) => {
      const targets = state.players.map((_, i) => i).filter(i => i !== playerIndex);
  ...
  minion: {
    ...
    type: ['Action', 'Attack'],
    cost: 5,
    description: '+1 アクション / 1つを選択する：(＋2 コイン); または (手札をすべて捨て、＋4 カード。他のプレイヤーは全員、手札が5枚以上ならそれらをすべて捨て、＋4 カード)。',
    effects: { action: 1, isAttack: true },
    ai: {
      getChoicePriorities: (state, playerIndex) => {
        const player = state.players[playerIndex]!;
        if (player.hand.filter(c => c.id === 'minion').length > 0) return ['+2 コイン', '手札入れ替え (+4 カード)'];
        const handVal = player.hand.reduce((sum, c) => sum + getAICardValueInHand(c, state, playerIndex, 'discard'), 0);
        if (handVal > 150) return ['+2 コイン', '手札入れ替え (+4 カード)'];
      ...
      helpers.setPrompt({
        ...
        onConfirm: (choice: string) => {
          if (choice.includes('コイン')) {
            player.coins += 2;
            helpers.addLog(`${player.name} は寵臣の効果で+2 コインを得ました。`);
          } else {
            ...
            helpers.addLog(`${player.name} は手札をすべて捨て、4枚引きました。`);

            state.players.forEach((p, i) => {
              if (i === playerIndex) return;
              if (p.hand.some(c => c.effects?.isDefense)) {
                helpers.addLog(`${p.name} は防御カードで防いだ！`);
                return;
              }
              if (p.hand.length >= 5) {
                ...
                helpers.addLog(`${p.name} は手札が5枚以上だったので、すべて捨てて4枚引きました。`);
              }
            });
          }
          helpers.clearPrompt();
  ...
  pawn: {
    ...
    type: ['Action'],
    cost: 2,
    description: '次のうち異なる2つを選択する：(＋1 カード); (＋1 アクション); (＋1 購入); (＋1 コイン)。',
    ai: {
      getChoicePriorities: (state, playerIndex) => {
        const priorities = [];
        if (state.players[playerIndex]!.actions === 0) priorities.push('+1 アクション');
        priorities.push('+1 カード');
        if (!priorities.includes('+1 アクション')) priorities.push('+1 アクション');
      ...
      helpers.setPrompt({
        ...
        onConfirm: (choices: string[]) => {
          choices.forEach(choice => {
            if (choice === '+1 カード') helpers.drawCards(playerIndex, 1);
            if (choice === '+1 アクション') player.actions += 1;
            if (choice === '+1 購入') player.buys += 1;
            if (choice === '+1 コイン') player.coins += 1;
          });
          helpers.addLog(`${player.name} は手先の効果で ${choices.join(', ')} を得ました。`);
          helpers.clearPrompt();
        }
      });
    }
  ...
  courtier: {
    ...
    type: ['Action'],
    cost: 5,
    description: '手札からカード1枚を公開する。そのカードの持っている種類（アクション、勝利点など）1つにつき、次の中から異なる1つを選択する。' +
                 '(＋1 アクション); (＋1 購入); (＋3 コイン); (金貨1枚を獲得する)。',
    ai: {
      getChoicePriorities: (state, playerIndex) => {
        const player = state.players[playerIndex]!;
        const sortedHand = [...player.hand].sort((a,b) => b.type.length - a.type.length);
        const priorities1 = sortedHand.map((c, i) => {
           const originalIdx = player.hand.indexOf(c);
           return `${originalIdx}: ${c.jpName}`;
        });
        const priorities2 = ['金貨を獲得', '+3 コイン', '+1 アクション', '+1 購入'];
    ...
    action: (state, playerIndex, helpers) => {
      ...
      helpers.setPrompt({
        ...
        onConfirm: (choice: string) => {
          ...
          helpers.addLog(`${player.name} は ${card.jpName} を公開しました（種類数: ${types}）。`);

          const options = ['+1 アクション', '+1 購入', '+3 コイン', '金貨を獲得'];
          ...
          helpers.setPrompt({
            ...
            onConfirm: (choices: string[]) => {
              choices.forEach(c => {
                ...
                if (c === '金貨を獲得') helpers.gainCard(playerIndex, 'gold');
              });
              helpers.addLog(`${player.name} は廷臣の効果で ${choices.join(', ')} を得ました。`);
              helpers.clearPrompt();
            }
          });
        }
      ...
      helpers.setPrompt({
        ...
        onConfirm: (indices: number[]) => {
          if (indices.length > 0) {
            ...
            helpers.addLog(`${player.name} はカードを1枚山札に戻しました。`);
          }
          helpers.clearPrompt();
        }
      });
  ...
  ironworks: {
    ...
    type: ['Action'],
    cost: 4,
    description: 'コスト4までのカード1枚を獲得する。獲得したカードが： アクションなら＋1 アクション; 財宝なら＋1 コイン; 勝利点なら＋1 カード。',
    action: (state, playerIndex, helpers) => {
      helpers.setPrompt({
        type: 'gain',
        targetPlayerIndex: playerIndex,
        ...
        onConfirm: (gainId: string) => {
          const currentCost = helpers.getCardCost(gainId, playerIndex);
          const targetCard = helpers.getCard(gainId);
          const allowedPotion = 0;
          if (currentCost <= 4 && (targetCard?.costPotion || 0) <= allowedPotion) {
            helpers.gainCard(playerIndex, gainId);
            ...
            if (gainedCard) {
              ...
              if (bonuses.length > 0) {
                helpers.addLog(`${p.name} は鉄工所の効果で ${bonuses.join(', ')} を得ました。`);
              }
            }
            helpers.clearPrompt();
          }
  ...
  coppersmith: {
    ...
    type: ['Action'],
    cost: 4,
    description: 'このターン、銅貨を場に出したとき、それは追加で1コインを生み出す。',
    action: (state, playerIndex, helpers) => {
      const player = state.players[playerIndex]!;
      player.turnFlags = player.turnFlags || {};
      player.turnFlags.coppersmithActive = (player.turnFlags.coppersmithActive || 0) + 1;
  ...
  scout: {
    ...
    type: ['Action'],
    cost: 4,
    description: '+1 アクション / 山札の上から4枚を公開する。その中の勝利点カードをすべて手札に加え、残りを好きな順で山札の上に戻す。',
    effects: { action: 1 },
    action: (state, playerIndex, helpers) => {
      const player = state.players[playerIndex]!;
      const revealed = helpers.revealCards(playerIndex, 4);
      ...
      if (backToDeck.length > 0) {
        if (backToDeck.length === 1) {
          ...
          helpers.setPrompt({
            ...
            min: backToDeck.length,
            max: backToDeck.length,
            message: '偵察員：山札に戻す順番を（上から順に）選択してください',
            onConfirm: (indices: number[]) => {
              const reordered = indices.map(idx => backToDeck[idx]!);
  ...
  mining_village: {
    ...
    type: ['Action'],
    cost: 4,
    description: '+1 カード, +2 アクション / あなたはこのカードを直ちに廃棄してもよい。そうした場合、＋2 コイン。',
      ...
      helpers.setPrompt({
        ...
        onConfirm: (choice: string) => {
          if (choice.includes('YES')) {
            const actualIndex = player.inPlay.findIndex(c => c.id === 'mining_village');
            if (actualIndex !== -1) {
              const card = player.inPlay.splice(actualIndex, 1)[0]!;
              state.trash.push(card);
              player.coins += 2;
              helpers.addLog(`${player.name} は鉱山の村を廃棄し、+2 コインを得ました。`);
            }
          }
          helpers.clearPrompt();
        }
  ...
  tribute: {
    ...
    type: ['Action'],
    cost: 5,
    description: '左隣のプレイヤーは自分の山札の上から2枚のカードを公開し、それらを捨て札にする。それらのカードの異なる種類ごとに、あなたは以下の利益を得る... (省略)',
    action: (state, playerIndex, helpers) => {
      const player = state.players[playerIndex]!;
      ...
      for(let i=0; i<2; i++) {
        if (leftPlayer.deck.length === 0) {
          if (leftPlayer.discard.length > 0) {
             ...
             helpers.addLog(`${leftPlayer.name} は捨て札をシャッフルして山札に戻しました。`);
          }
        }
      ...
      if (revealed.length > 0) {
         helpers.addLog(`${leftPlayer.name} は ${revealed.map(c => c.jpName).join('、')} を公開して捨て札にしました。`);
         const typesFound = new Set<string>();
         revealed.forEach(c => c.type.forEach(t => typesFound.add(t)));
         const bonuses: string[] = [];
         ...
         if (bonuses.length > 0) {
           helpers.addLog(`${player.name} は貢物の効果で ${bonuses.join('、')} を得ました。`);
         }
      }
    }
  },
  secret_chamber: {
    ...
    cost: 2,
    description: '好きな枚数のカードを捨て札にする。捨て札にしたカード1枚につき+1コイン / アタックに対するリアクション...',
    action: (state, playerIndex, helpers) => {
      const player = state.players[playerIndex]!;
      if (player.hand.length === 0) return;
      helpers.setPrompt({
        ...
        message: '秘密の部屋：捨てるカードを選択してください（1枚につき+1コイン）',
        onConfirm: (indices: number[]) => {
          if (indices.length > 0) {
            helpers.discardSelected(playerIndex, indices);
            player.coins += indices.length;
            helpers.addLog(`${player.name} は ${indices.length} 枚捨て、+${indices.length} コインを得ました。`);
          }
          helpers.clearPrompt();
        }
      });
    ...
    ai: {
      shouldReact: (state, playerIndex) => {
        const player = state.players[playerIndex]!;
        const victoryCards = player.hand.filter(c => c.type.includes('Victory')).length;
    ...
    onAttackReaction: (state, victimIndex, helpers) => {
      ...
      helpers.setPrompt({
        ...
        min: 2,
        max: 2,
        message: '秘密の部屋：山札の番上に置く手札2枚を選択してください',
        onConfirm: (indices: number[]) => {
          const sorted = [...indices].sort((a,b) => b-a);
          sorted.forEach(idx => {
            const card = player.hand.splice(idx, 1)[0]!;
          ...
          helpers.addLog(`${player.name} は手札から2枚を山札の上に戻しました。`);
          helpers.clearPrompt();
        }
      });
    }
  ...
  bridge: {
    ...
    type: ['Action'],
    cost: 4,
    description: '+1 購入, +1 コイン / このターン、全てのカードのコストは 1 下がる。',
    ...
    action: (state, playerIndex, helpers) => {
      ...
      helpers.addLog(`${player.name} は橋の効果でカードのコストを 1 下げました。`);
    }
  },
  diplomat: {
    id: 'diplomat',
    ...
    type: ['Action', 'Reaction'],
    cost: 4,
    description: '+2 カード / デッキからカードを引いた後, 手札が5枚以下なら +2 アクション / '
    ...
    action: (state, playerIndex, helpers) => {
      ...
      if (player.hand.length <= 5) {
        state.players[playerIndex]!.actions += 2;
        helpers.addLog(`${player.name} は手札が5枚以下だったので、+2アクションが有効になりました。`);
      }
    },
    onAttackReaction: (state, victimIndex, helpers) => {
      const player = state.players[victimIndex]!;
  ...
  wishing_well: {
    ...
    type: ['Action'],
    cost: 3,
    description: '+1 カード, +1 アクション / カード名を宣言し、山札の上がそれなら手札に加える。',
    effects: { draw: 1, action: 1 },
    ai: {
      getChoicePriorities: (state, playerIndex) => {
        const player = state.players[playerIndex]!;
        ...
        const sortedCards = Object.entries(cardRecord).sort((a, b) => b[1] - a[1]);
    ...
    action: (state, playerIndex, helpers) => {
      ...
      helpers.setPrompt({
        ...
        message: '願いの井戸：山札の一番上にあると思うカードを選択してください',
        options: supplyNames,
        onConfirm: (guessedId: string) => {
          ...
          if (player.deck.length > 0) {
            ...
            helpers.addLog(`${player.name} は山札の一番上のカードを公開しました：${topCard.jpName}`);
  ...
  masquerade: {
    ...
    ai: {
      getChoicePriorities: (state, playerIndex) => {
        const player = state.players[playerIndex]!;
        const hand = [...player.hand].map((c, i) => ({ c, i, v: getAICardValueInHand(c, state, playerIndex, 'trash') }));
    ...
    action: (state, playerIndex, helpers) => {
      ...
      helpers.processAllPlayers?.((idx, next) => {
        const p = state.players[idx]!;
        if (p.hand.length === 0) { next(); return; }
        helpers.setPrompt({
          type: 'choice',
        ...
        if (currentPlayerObj.hand.length > 0) {
          helpers.setPrompt({
            ...
            min: 0,
            max: 1,
            message: '仮面舞踏会：手札から1枚廃棄してもよい（任意）',
            onConfirm: (indices: number[]) => {
              if (indices.length > 0) {
                const card = currentPlayerObj.hand.splice(indices[0]!, 1)[0]!;
                state.trash.push(card);
                helpers.addLog(`${currentPlayerObj.name} は仮面舞踏会の効果で ${card.jpName} を廃棄しました。`);
              }
              helpers.clearPrompt();
            }
          });
  ...
  saboteur: {
    ...
    type: ['Action', 'Attack'],
    cost: 5,
    description: 'コスト3以上のカードが公開されるまで公開し廃棄する。2低いカードを獲得してもよい。',
      ...
      helpers.processAttack?.('saboteur', (victimIdx) => {
        ...
        const revealNext = () => {
          ...
          if (victim.deck.length > 0) {
            ...
            const currentCost = helpers.getCardCost(card.id, playerIndex);
            if (currentCost >= 3) {
              state.trash.push(card);
              helpers.addLog(`${victim.name} は ${card.jpName} を廃棄しました。`);
              ...
              if (maxGainCost >= 0) {
                helpers.setPrompt({
                  ...
                  onConfirm: (gainId: string) => {
                    if (gainId) helpers.gainCard(victimIdx, gainId);
                    helpers.clearPrompt();
                  }
                });
    ...
    action: (state, playerIndex, helpers) => {
      helpers.processAttack?.('swindler', (victimIdx) => {
        ...
        if (victim.deck.length > 0) {
          ...
          const trashedCost = helpers.getCardCost(cardToTrash.id, playerIndex);
          helpers.addLog(`${victim.name} の ${cardToTrash.jpName} が廃棄されました。`);
          helpers.setPrompt({
            type: 'gain',
            targetPlayerIndex: playerIndex,
            cardId: 'swindler',
