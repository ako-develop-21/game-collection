...
export const PROSPERITY_CARDS: Record<string, Card> = {
  ...
  royal_seal: {
    ...
    type: ['Treasure'],
    cost: 5,
    description: '+2 コイン / このカードが場に出ているかぎり、あなたがカード1枚を獲得するとき、そのカードを自分の山札の一番上に置いてもよい。',
    effects: { coin: 2 },
    onBuy: (state, playerIndex, helpers, cardItem, cardIndex) => {
      const player = state.players[playerIndex]!;
        ...
        helpers.setPrompt({
          ...
          onConfirm: (choice: string) => {
            if (choice === 'はい' || choice === 'YES') {
              ...
              const bottomCard = player.discard[player.discard.length - 1];
              if (!bottomCard || bottomCard.id !== cardItem.id) {
                helpers.addLog(`${cardItem.id} is not found in discard!!!`);
              } else {
                player.discard.splice(player.discard.length - 1, 1);
                player.deck.push(cardItem);
                player.turnFlags = { ...player.turnFlags, gainCardToDeck: true }
              }
            }
            helpers.clearPrompt();
          }
  ...
  quarry: {
    ...
    type: ['Treasure'],
    cost: 4,
    description: '+1 コイン / このターン、アクションカードのコストは2少なくなる。',
    effects: { coin: 1 },
    onPlay: (state, playerIndex, helpers) => {
      const player = state.players[playerIndex]!;
      player.turnFlags = { 
        ...player.turnFlags, 
        costActionReduction: (player.turnFlags?.costActionReduction || 0) + 2 
      };
      helpers.addLog(`${player.name} は石切場の効果でアクションカードのコストを 2 下げました。`);
    }
  },
  anvil: {
    id: 'anvil',
    ...
    type: ['Treasure'],
    cost: 3,
    description: '+1 コイン / あなたは財宝カード1枚を捨て札にしてもよい。そうした場合コスト4以下のカード1枚を獲得する。',
    effects: { coin: 1 },
    onPlay: (state, playerIndex, helpers) => {
      const player = state.players[playerIndex]!;
      const handTreasures = player.hand.filter(c => c.type.includes('Treasure'));
          ...
          if (indices.length > 0) {
            ...
            helpers.setPrompt({
              ...
              onConfirm: (gainCardId: string) => {
                const currentGainCost = helpers.getCardCost(gainCardId, playerIndex);
                const targetCard = helpers.getCard(gainCardId);
                if (currentGainCost <= 4 && (targetCard?.costPotion || 0) <= 0) {
                  helpers.gainCard(playerIndex, gainCardId);
                  helpers.clearPrompt();
                }
              }
