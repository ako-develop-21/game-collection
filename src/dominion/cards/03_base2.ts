...
export const BASE2_CARDS: Record<string, Card> = {
  remodel: {
    ...
    type: ['Action'],
    cost: 4,
    description: '手札からカード1枚を廃棄する。そのカードよりコストが最大2多いまでのカード1枚を獲得する。',
    ai: {
      getChoicePriorities: (state, playerIndex) => {
        const player = state.players[playerIndex]!;
        const trashCount = 1;
        const handScore = player.hand.map(c => {
          return { c, score: getAICardValueInHand(c, state, playerIndex, 'trash') }
          ...
          if (card) {
            ...
            helpers.setPrompt({
              ...
              onConfirm: (gainCardId: string) => {
                const currentGainCost = helpers.getCardCost(gainCardId, playerIndex);
                const targetCard = helpers.getCard(gainCardId);
                if (currentGainCost <= costLimit && (targetCard?.costPotion || 0) <= trashedPotion) {
  ...
  chapel: {
    ...
    ai: {
      getDiscardTrashIndices: (state, playerIndex) => {
        ...
        // 通常の手札価値を取得
        const indices = player.hand.map((c, i) => {
          return { c, value: getAICardValueInHand(c, state, playerIndex, 'trash'), index: i }
        })
        .filter(v => v.value < 0)
  ...
  mine: {
    ...
    type: ['Action'],
    cost: 5,
    description: '手札から財宝カード1枚を廃棄する。そのカードよりコストが最大3多いまでの財宝カード1枚を獲得し、手札に加える。',
    action: (state, playerIndex, helpers) => {
      const player = state.players[playerIndex]!;
      const treasureIndices = player.hand.map((c, i) => c.type.includes('Treasure') ? i : -1).filter(i => i !== -1);
  ...
  gardens: {
    ...
    type: ['Victory'],
    cost: 4,
    description: 'あなたのデッキのカード10枚（端数切り捨て）につき、1 勝利点。',
    points: (state, playerIndex) => {
      const player = state.players[playerIndex]!;
      const totalCards = player.hand.length + player.deck.length + player.discard.length + player.inPlay.length;
  ...
  artisan: {
    ...
    type: ['Action'],
    cost: 6,
    description: 'コスト5までのカード1枚を獲得し、手札に加える。手札からカード1枚を山札の一番上に置く。',
    action: (state, playerIndex, helpers) => {
      helpers.setPrompt({
        type: 'gain',
        targetPlayerIndex: playerIndex,
        ...
        onConfirm: (gainCardId: string) => {
          ...
          if (currentGainCost <= 5 && (targetCard?.costPotion || 0) <= allowedPotion) {
            helpers.gainCard(playerIndex, gainCardId, 'hand');
            ...
            helpers.setPrompt({
              ...
              min: 1,
              max: 1,
              message: '職人：山札の上に戻すカードを選択してください',
               onConfirm: (indices: number[]) => {
                const player = state.players[playerIndex]!;
                const card = player.hand[indices[0]!];
                if (card) {
                  ...
                  helpers.addLog(`${player.name} は手札から1枚山札の上に戻しました。`);
                }
                helpers.clearPrompt();
              }
            });
  ...
  bandit: {
    ...
    type: ['Action', 'Attack'],
    cost: 5,
    description: '金貨1枚を獲得する。他のプレイヤーは山札の上から2枚を公開し、銅貨以外の財宝カードを1枚廃棄し、残りを捨て札にする。',
    ...
    action: (state, playerIndex, helpers) => {
      ...
      helpers.processAttack('bandit', (victimIdx) => {
        ...
        const trashableIndices = revealed.map((c, idx) => c.type.includes('Treasure') && c.id !== 'copper' ? idx : -1).filter(idx => idx !== -1);
        
        if (trashableIndices.length > 0) {
          trashableIndices.sort((a, b) => (revealed[b]?.cost || 0) - (revealed[a]?.cost || 0));
          const trashIdx = trashableIndices[0]!;
          const trashedCard = revealed[trashIdx]!;
          state.trash.push(trashedCard);
          helpers.addLog(`${victim.name} は ${trashedCard.jpName} を廃棄しました。`);
  ...
  bureaucrat: {
    ...
    type: ['Action', 'Attack'],
    cost: 4,
    description: '銀貨1枚を獲得し、山札の上に置く。他のプレイヤーは手札から勝利点カード1枚を山札の上に置く（持っていなければ手札を公開する）。',
    action: (state, playerIndex, helpers) => {
      ...
      const processTarget = (idx: number) => {
        ...
        const victoryIndices = player.hand.map((c, idx) => c.type.includes('Victory') ? idx : -1).filter(idx => idx !== -1);
        if (victoryIndices.length > 0) {
          helpers.setPrompt({
            ...
            min: 1,
            max: 1,
            message: '役人：山札の上に戻す勝利点カードを選択してください',
            onConfirm: (indices: number[]) => {
              const targetIdx = indices[0]!;
              const card = player.hand.splice(targetIdx, 1)[0]!;
              player.deck.push(card);
              helpers.addLog(`${player.name} は ${card.jpName} を山札の上に戻しました。`);
              next();
            }
          });
        } else {
          helpers.addLog(`${player.name} は勝利点カードを持っていないため、手札を公開しました: ${player.hand.map(c => c.jpName).join(', ')}`);
          next();
        }
      };
  ...
  harbinger: {
    ...
    type: ['Action'],
    cost: 3,
    description: '+1 カード, +1 アクション / 捨て札からカードを1枚見て、山札の一番上に置いてもよい。',
        ...
        onConfirm: (selectedIndices: number[]) => {
          if (selectedIndices.length > 0) {
            ...
            if (card) {
              ...
              player.deck.push(card);
              helpers.addLog(`${player.name} は捨て札から ${card.jpName} を山札の上に戻しました。`);
            }
          }
          helpers.clearPrompt();
        }
  ...
  library: {
    ...
    type: ['Action'],
    cost: 5,
    description: '手札が7枚になるまで引く。アクションカードを引いた場合、それを脇に置いてもよい。脇に置かなかったカードは手札に加え、脇に置いたカードは最後に捨て札にする。',
    ai: {
      getChoicePriorities: (state, playerIndex) => {
        const player = state.players[playerIndex]!;
        // Decision: YES (aside) / NO (into hand)
        if (player.hand.length < 5) return ['NO (手札に入れる)', 'YES (脇に置く)'];
      ...
      const drawStep = () => {
        if (player.hand.length >= 7) {
          if (asideCards.length > 0) {
            ...
            helpers.addLog(`${player.name} は脇に置いた ${asideCards.length} 枚のカードを捨て札にしました。`);
          }
          return;
        }
        ...
        if (revealed.length === 0) {
          if (asideCards.length > 0) player.discard.push(...asideCards);
          return;
        }
          ...
          helpers.setPrompt({
            ...
            onConfirm: (choice: string) => {
              if (choice.startsWith('YES')) {
                ...
                helpers.addLog(`${player.name} は ${card.jpName} を脇に置きました。`);
              } else {
  ...
  merchant: {
    ...
    type: ['Action'],
    cost: 3,
    description: '+1 カード, +1 アクション / このターン最初に銀貨をプレイしたとき、+1 コイン。',
    effects: { draw: 1, action: 1 },
    action: (state, playerIndex, helpers) => {
      state.players[playerIndex]!.turnFlags = { ...state.players[playerIndex]!.turnFlags, merchantActive: true };
    }
  },
  moneylender: {
    id: 'moneylender',
    ...
    type: ['Action'],
    cost: 4,
    description: '手札から銅貨1枚を廃棄してもよい。そうした場合、+3 コイン。',
    action: (state, playerIndex, helpers) => {
      const player = state.players[playerIndex]!;
      ...
      if (copperIdx !== -1) {
        ...
        player.coins += 3;
        helpers.addLog(`${player.name} は銅貨を自動廃棄し、3コインを得ました。`);
      }
    }
  },
  throne_room: {
    ...
    type: ['Action'],
    cost: 4,
    description: '手札からアクションカード1枚を選択する。そのカードを2回プレイする。',
    action: (state, playerIndex, helpers) => {
      const player = state.players[playerIndex]!;
      const actionIndices = player.hand.map((c, i) => c.type.includes('Action') ? i : -1).filter(i => i !== -1);
      ...
      helpers.setPrompt({
        ...
        min: 1,
        max: 1,
        message: '玉座の間：2回プレイするアクションカードを選択してください',
        onConfirm: (indices: number[]) => {
          ...
          if (card && card.type.includes('Action')) {
            ...
            helpers.addLog(`${player.name} は玉座の間で ${card.jpName} を2回プレイします。`);
            ...
            const executeSequentially = (remaining: number) => {
              ...
              // Apply effects
              if (card.effects) {
                if (card.effects.draw) helpers.drawCards(playerIndex, card.effects.draw)
              ...
              if (activePrompt) {
                hookPrompt(activePrompt, () => executeSequentially(remaining - 1));
              } else {
  ...
  vassal: {
    ...
    type: ['Action'],
    cost: 3,
    description: '+2 コイン / 山札の一番上のカードを捨て札にする。それがアクションカードの場合、それをプレイしてもよい。',
    ai: {
          ...
          helpers.setPrompt({
            ...
            onConfirm: (choice: string) => {
              if (choice === 'YES') {
                ...
                // Play logic
                player.inPlay.push(card);
                helpers.addLog(`${player.name} は家臣の効果で ${card.jpName} をプレイしました。`);
                ...
                helpers.addLog(`${player.name} は ${card.jpName} をプレイせずに捨て札にしました。`);
              }
            }
          });
        } else {
          player.discard.push(card);
          helpers.addLog(`${player.name} は ${card.jpName} を捨て札にしました。`);
        }
      }
    }
  },
    ...
    action: (_state, playerIndex, helpers) => {
      helpers.setPrompt({
        ...
        onConfirm: (cardId: string) => {
          const currentCost = helpers.getCardCost(cardId, playerIndex);
