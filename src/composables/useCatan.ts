...
export function useCatan() {
  ...
  const playDevelopmentCard = (
    ...
    addLog(
      playerId,
      "devCard",
      `${player.name} played ${card.type.replace("_", " ").toUpperCase()}`,
    );
      ...
      case "year_of_plenty":
        if (data?.resources) {
          data.resources.forEach(
            (r: ResourceType) => player.resources[r as ResourceType]++,
          );
        }
        break;
      case "monopoly":
  ...
  const generateBoard = () => {
    ...
    for (let q = -2; q <= 2; q++) {
      for (let r = Math.max(-2, -q - 2); r <= Math.min(2, -q + 2); r++) {
        ...
        const num =
          resource === "desert" ? null : shuffledNumbers[numberIdx++]!;
  ...
  const deductCost = (playerId: number, type: string) => {
    ...
    (Object.entries(cost) as [ResourceType, number][]).forEach(([res, val]) => {
  ...
  const checkWinner = () => {
    ...
    const winner = players.value.find((p) => calculateTotalPoints(p.id) >= 10);
    if (winner) {
  ...
  const getDominanceScore = (playerId: number) => {
    ...
    const cities = Object.values(nodes.value).filter(
      (n) => n.building?.type === "city" && n.building.playerId === playerId,
    ).length;
  ...
  const updateAwards = () => {
    ...
    if (
      currentLargestArmy.playerId !== awardHolders.value.largestArmy.playerId
    ) {
      const winner = players.value.find(
        (p) => p.id === currentLargestArmy.playerId,
      );
      if (winner)
        addLog(
          currentLargestArmy.playerId,
          "award",
          `${winner.name} took Largest Army!`,
        );
    }
  ...
  const calculateLongestRoad = (playerId: number): number => {
    ...
    const getNeighbors = (edgeId: string) => {
      ...
      if (
        node1 &&
        (node1.building === null || node1.building.playerId === playerId)
      ) {
      ...
      if (
        node2 &&
        (node2.building === null || node2.building.playerId === playerId)
      ) {
        playerEdges.forEach((e) => {
          if (
            e.id !== edgeId &&
    ...
    const dfs = (edgeId: string, visited: Set<string>): number => {
      ...
      for (const neighbor of neighbors) {
        if (!visited.has(neighbor)) {
          currentMax = Math.max(currentMax, dfs(neighbor, new Set(visited)));
        }
      }
      return 1 + currentMax;
    };
  ...
  const buildRoad = (edgeId: string, playerId: number, free = false) => {
    ...
    edge.road = { playerId };
    const player = players.value.find((p) => p.id === playerId);
    addLog(playerId, "build", `🚧 ${player?.name} built a Road`);
    updateAwards();
  ...
  const buildSettlement = (nodeId: string, playerId: number, free = false) => {
    const isSetup = setupPhase.value !== "none";
    if (!free && !isSetup && !canAfford(playerId, "settlement")) return false;
    ...
    if (player) {
      player.points += 1;
      addLog(playerId, "build", `🏠 ${player.name} built a Settlement`);
  ...
  const buildCity = (nodeId: string, playerId: number) => {
    ...
    if (player) {
      ...
      addLog(playerId, "build", `🏛️ ${player.name} upgraded to a City`);
      checkWinner();
    }
    return true;
  };
  ...
  const rollDice = () => {
          ...
          if (pid !== 0) {
            ...
            if (p) {
              ...
              const currentResources = (
                Object.entries(p.resources) as [ResourceType, number][]
              ).flatMap(([res, count]) => Array(count).fill(res));
              ...
              kept.forEach((r) => p.resources[r as ResourceType]++);
    ...
    addLog(null, "dice", `🎲 Dice rolled: ${d1 + d2} (${d1}, ${d2})`);
  ...
  const aiPlayTurn = async () => {
    ...
    if (setupPhase.value !== "none") {
      ...
      if (setupStep.value === "settlement") {
        const validNodes = Object.keys(nodes.value).filter((id) => {
          ...
          const isTooClose = Object.keys(edges.value).some((eid) => {
            if (eid.includes(id)) {
              const otherNodeId = eid.split("--").find((oid) => oid !== id);
              return otherNodeId && nodes.value[otherNodeId]?.building !== null;
            }
            return false;
          });
          return !isTooClose;
        ...
        if (setupPhase.value === "second") {
          ...
          if (firstSettlement) {
            const firstResources = hexes.value
              .filter((h) => hexToNodes.get(h.id)?.includes(firstSettlement.id))
              .map((h) => h.resource);

            // Boost value of resources we DON'T have yet
            (
              ["wood", "brick", "wool", "wheat", "ore"] as ResourceType[]
        ...
        validNodes.forEach((id) => {
          ...
          if (node && node.port === null) {
            const isCoast =
              hexes.value.filter((hex) => hexToNodes.get(hex.id)?.includes(id))
                .length < 3;
            if (isCoast) score -= 2;
        ...
        validEdges.forEach((eid) => {
          ...
          if (otherNodeId) {
            ...
            Object.keys(edges.value).forEach((nextEid) => {
              if (nextEid.includes(otherNodeId) && nextEid !== eid) {
                const targetNodeId = nextEid
                  .split("--")
                  .find((nid) => nid !== otherNodeId);
                if (targetNodeId) {
                  edgeScore += getIntersectionScore(targetNodeId, p.id);
                }
              }
            });
    ...
    if (turnPhase.value === "robber") {
      ...
      const otherHexes = hexes.value.filter((h) => h !== currentRobberHex);
      const targetHex =
    ...
    const canPlayCard = (type: DevCardType) => {
      const cardIdx = player.devCards.findIndex(
        (c) => c.type === type && !c.played && c.turnBought < turnCount.value,
      );
      return cardIdx;
    };
      ...
      if (pickedResources.length < 2) {
        const sorted = (Object.keys(player.resources) as ResourceType[])
          ...
          .sort((a, b) => {
            ...
            return player.resources[a] / wa - player.resources[b] / wb;
          });
    ...
    if (knightIdx !== -1) {
      ...
      const isBlockingMe = nodesInHex.some(
        (nid) => nodes.value[nid]?.building?.playerId === currentPlayerId.value,
      );
      ...
      if (targetEdge) {
        ...
        const validEdges = Object.keys(edges.value).filter((eid) => {
          ...
          return (
            ...
            nodesInEdge.some(
              (nid) =>
                nodes.value[nid]?.building?.playerId === currentPlayerId.value,
            )
          );
        });
        const randomEdge =
    ...
    const proactiveBankTrade = () => {
      ...
      for (const type of targetTypes) {
        ...
        const missingResources = (Object.keys(cost) as ResourceType[]).filter(
          (res) => player.resources[res] < cost[res],
        );
        if (missingResources.length === 1) {
          const missingRes = missingResources[0]!;
          const required = cost[missingRes] - player.resources[missingRes];

          if (required === 1) {
            // Find a resource we can sell
            for (const res of Object.keys(player.resources) as ResourceType[]) {
              if (res === missingRes || res === "desert") continue;
              const ratio = getTradeRatio(player.id, res);
              const surplus = player.resources[res] - cost[res];
              if (surplus >= ratio) {
                // Perform trade!
                if (executeBankTrade(player.id, res, missingRes)) return true;
              }
            }
          }
        }
  ...
  const executeBankTrade = (
    ...
    addLog(
      playerId,
      "trade",
      `🤝 ${player.name} traded ${ratio} ${sellRes} for 1 ${buyRes} with Bank`,
    );
    return true;
  };
  ...
  const evaluatePlayerTrade = (
    ...
    const getTradeValue = (resMap: Partial<Record<ResourceType, number>>) => {
  ...
  const discardResources = (
    ...
    // Execute
    (Object.entries(resourcesToDiscard) as [ResourceType, number][]).forEach(
      ([res, count]) => {
  ...
  const findBestTargetNode = (playerId: number): string | null => {
    ...
    Object.keys(nodes.value).forEach((nodeId) => {
      ...
      const isTooClose = Object.keys(edges.value).some((eid) => {
        if (eid.includes(nodeId)) {
          const otherNodeId = eid.split("--").find((oid) => oid !== nodeId);
          return otherNodeId && nodes.value[otherNodeId]?.building !== null;
        }
        return false;
      });
  ...
  const getPathToNode = (playerId: number, targetNodeId: string): string[] => {
    ...
    while (queue.length > 0) {
      ...
      Object.keys(edges.value).forEach((eid) => {
        if (eid.includes(nodeId)) {
          const neighborId = eid.split("--").find((oid) => oid !== nodeId);
