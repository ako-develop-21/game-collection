...
import {
    getAICardValueInHand,
    getAIPurchaseWeight,
} from "../dominion/aiHelpers";
import type {
    GameState,
    Card,
    PlayerState,
...
export function useDominion() {
    ...
    const drawCards = (playerIndex: number, count: number) => {
        ...
        for (let i = 0; i < count; i++) {
            if (player.deck.length === 0) {
                ...
                shuffle(player.deck);
                addLog(
                    `${player.name} は捨て札をシャッフルして山札に戻しました。`,
                );
            }
    ...
    const getCardCost = (cardId: string, playerIndex: number): number => {
        ...
        if (!card) return 0;
        const reduction =
            players.value[playerIndex]?.turnFlags?.costReduction || 0;
        const actionReduction =
            players.value[playerIndex]?.turnFlags?.costActionReduction || 0;
    ...
    const processAttack = (
        attackCardId: string,
        victimAction: (victimIdx: number) => void,
    ) => {
        ...
        const processVictim = (idx: number) => {
            ...
            if (defenseCard) {
                addLog(
                    `${victim.name} は ${defenseCard.jpName} を公開し、アタックを防いだ！`,
                );
                processVictim(idx + 1);
                return;
            }
                ...
                const handleReaction = (reactionIdx: number) => {
                    ...
                    const askReaction = () => {
                        ...
                        if (victim.name.startsWith("AI")) {
                            const shouldReact = card.ai?.shouldReact
                                ? card.ai.shouldReact(
                                      state,
                                      victimIdx,
                                      attackCardId,
                            ...
                            if (shouldReact) {
                                addLog(
                                    `${victim.name} は ${card.jpName} でリアクションした！`,
                                );
                                card.onAttackReaction?.(
                                    state,
                                    victimIdx,
                                    helpers,
                                );
                        ...
                        setPrompt({
                            ...
                            onConfirm: (choice: string) => {
                                if (choice.includes("YES")) {
                                    addLog(
                                        `${victim.name} は ${card.jpName} を公開した！`,
                                    );
                                    card.onAttackReaction?.(
                                        state,
                                        victimIdx,
    ...
    const processAllPlayers = (
        action: (idx: number, next: () => void) => void,
        onComplete?: () => void,
    ) => {
    ...
    const checkActionPhase = () => {
        ...
        const hasActionCard = player.hand.some((c) =>
            c.type.includes("Action"),
        );
        if (!hasActionCard || player.actions === 0) {
            if (!currentPrompt.value) {
                // Don't end phase if a prompt is active
                endActionPhase();
            }
        }
    };
    ...
    const runAITurn = async () => {
        if (isGameOver.value || !currentPlayer.value.name.startsWith("AI"))
            return;
        ...
        while (
            phase.value === "Action" &&
            !isGameOver.value &&
            currentPlayer.value.name.startsWith("AI")
        ) {
            ...
            const actionCards = currentPlayer.value.hand
                .map((c, i) => ({
                    c,
                    i,
                    weight: getAIActionWeight(c, currentPlayer.value.actions),
                }))
            ...
            if (actionCards.length > 0 && currentPlayer.value.actions > 0) {
                ...
                addLog(
                    `AI は ${topAction.jpName} をプレイすることを選択しました。`,
                );
            ...
            while (
                ...
                const candidates = Object.keys(supply.value)
                    ...
                    .filter((item) => {
                        ...
                        return (
                            supply.value[item.id]! > 0 &&
                            item.weight > 0 &&
                            currentPlayer.value.coins >= cost &&
                            currentPlayer.value.potions >= potionCost
                        );
                    })
                    .sort((a, b) => b.weight - a.weight);
    ...
    watch(currentPrompt, async (newPrompt: ActionPrompt | null) => {
        if (
            newPrompt &&
            players.value[newPrompt.targetPlayerIndex]?.name.startsWith("AI")
        ) {
            ...
            if (newPrompt.type === "sentry") {
                ...
                // --------------------------------------------------------------------------------
                const decisions =
                    newPrompt.revealedCards?.map((card) => {
                        const val = getAICardValueInHand(
                            card,
                            state,
                            newPrompt.targetPlayerIndex,
                            newPrompt.type,
                        );
                        if (val < 0) return { card, action: "trash" as const };
                        if (val < 20)
                            return { card, action: "discard" as const };
                        return { card, action: "putBack" as const };
                    }) || [];

                newPrompt.onConfirm(decisions);
            } else if (
                newPrompt.type === "discard" ||
                newPrompt.type === "trash"
            ) {
                // --------------------------------------------------------------------------------
                // 捨て札、廃棄
                // --------------------------------------------------------------------------------
                const card = newPrompt.cardId
                    ? ALL_CARDS[newPrompt.cardId]
                    : null;

                if (card?.ai?.getDiscardTrashIndices) {
                    const indices = card.ai.getDiscardTrashIndices(
                        state,
                        newPrompt.targetPlayerIndex,
                        newPrompt as any,
                    );
                ...
                const handWithIndices = player.hand.map((card, index) => ({
                    card,
                    index,
                ...
                // --------------------------------------------------------------------------------
                const cards =
                    newPrompt.revealedCards &&
                    newPrompt.revealedCards.length > 0
                        ? newPrompt.revealedCards
                        : players.value[newPrompt.targetPlayerIndex]!.hand;
                const items = cards.map((c, i) => ({
                    c,
                    i,
                    v: getAICardValueInHand(
                        c,
                        state,
                        newPrompt.targetPlayerIndex,
                        "reorder",
                ...
                if (
                    ...
                    newPrompt.onConfirm(
                        resultIndices.slice(0, newPrompt.max || 1),
                    );
                } else {
                ...
                // --------------------------------------------------------------------------------
                const allowedCost =
                    newPrompt.allowedCost !== undefined
                        ? newPrompt.allowedCost
                        : 99;

                const choices = Object.keys(supply.value).filter((cardId) => {
                    const c = ALL_CARDS[cardId];
                    if (!c || supply.value[cardId] === 0) return false;
                    const currentCost = getCardCost(
                        cardId,
                        newPrompt.targetPlayerIndex,
                    );
                    if (currentCost > allowedCost) return false;
                    if (newPrompt.exactCost && currentCost !== allowedCost)
                        return false;
                    if (
                        newPrompt.allowedTypes &&
                        !newPrompt.allowedTypes.some((t) => c.type.includes(t))
                    )
                        return false;
                    return true;
                });
                ...
                if (choices.length > 0) {
                    ...
                    const bestWeight = getAIPurchaseWeight(
                        bestCardId,
                        99,
                        state,
                        newPrompt.targetPlayerIndex,
                ...
                if (newPrompt.maxChoices !== undefined) {
                    ...
                    // Final safety fill
                    while (
                        selected.length < (newPrompt.maxChoices || 0) &&
                        selected.length < options.length
                    ) {
                        const nextMatch = options.find(
                    ...
                    if (card?.ai?.getChoicePriorities) {
                        ...
                        const match = priorities.find((opt) =>
                    ...
                    const choice =
                        player.hand.length >= 6 ? options[1] : options[0];
    ...
    const initGame = (
        aiCount: number = 1,
        explicitlySelectedCards?: string[],
    ) => {
        ...
        selectedKingdomCards.sort(
            (a, b) => ALL_CARDS[a]!.cost - ALL_CARDS[b]!.cost,
        );
        ...
        const needsPotion = selectedKingdomCards.some(
            (id) => ALL_CARDS[id]?.costPotion,
        );
        if (needsPotion) {
            supply.value["potion"] = 16;
        }

        const needsColony = selectedKingdomCards.some(
            (id) => PROSPERITY_CARDS[id],
        );
        if (needsColony) {
    ...
    const gainCard = (
        playerIndex: number,
        cardId: string,
        destination: "discard" | "hand" | "deck" = "discard",
    ) => {
        const player = players.value[playerIndex]!;
        const card = ALL_CARDS[cardId];
        if (!card) return;
    ...
    const playCard = (cardIndex: number) => {
        ...
        if (phase.value === "Action" && card.type.includes("Action")) {
            if (player.actions > 0) {
                ...
                if (card.effects) {
                    if (card.effects.draw)
                        drawCards(currentPlayerIndex.value, card.effects.draw);
                    ...
                    if (card.effects.vpTokens)
                        player.vpTokens += card.effects.vpTokens;
                }
            ...
            // Merchant check
            if (card.id === "silver") {
                const merchantCount = player.inPlay.filter(
                    (c) => c.id === "merchant",
                ).length;
                if (merchantCount > 0 && !player.turnFlags?.silverPlayed) {
                    player.coins += merchantCount;
                    addLog(
                        `${player.name} は商人の効果で +${merchantCount} コインを得ました。`,
                    );
                    player.turnFlags = {
                        ...player.turnFlags,
                        silverPlayed: true,
            ...
            if (earnedPotions > 0)
                msgParts.push(`+${earnedPotions} ポーション`);
    ...
    const buyCard = (cardId: string) => {
        ...
        if (
            ...
            if (player.turnFlags?.gainCardToDeck)
                player.turnFlags.gainCardToDeck = false;
    ...
    const endTurn = () => {
        ...
        phase.value = "Action";
        addLog(
            `${players.value[currentPlayerIndex.value]!.name} のターンです。`,
        );
    ...
    const checkGameOver = () => {
        const emptyPiles = Object.values(supply.value).filter(
            (count) => count === 0,
        ).length;
        const lostProvince = supply.value["province"] === 0;
        const lostColony =
            typeof supply.value["colony"] === "number" &&
            supply.value["colony"] === 0;
        if (lostProvince || lostColony || emptyPiles >= 3) {
            ...
            const maxPoints = Math.max(
                ...players.value.map((p) => p.totalPoints),
            );
            winners.value = players.value.filter(
                (p) => p.totalPoints === maxPoints,
            );

            if (winners.value.length === 1 && winners.value[0]) {
                addLog(
                    `勝者: ${winners.value[0].name} (${winners.value[0].totalPoints}点)`,
                );
            } else if (winners.value.length > 1) {
                addLog(
                    `引き分け: ${winners.value.map((p) => p.name).join(", ")} (${maxPoints}点)`,
                );
            }
            return true;
        }
