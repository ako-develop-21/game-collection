...
const isSameValue = (cell: SudokuCell) => {
  ...
  const selected = props.grid?.[props.selectedCell.row]?.[props.selectedCell.col]
...
.sudoku-cell {
  ...
  &.border-right { border-right: 2px solid rgba(255, 255, 255, 0.3); }
  &.border-bottom { border-bottom: 2px solid rgba(255, 255, 255, 0.3); }

  &.original { color: white; font-weight: 700; }
  &.error { 
  ...
  &.related { background: rgba(255, 255, 255, 0.03); }
  &.same-value { background: rgba(74, 144, 226, 0.15); }
  &.selected { background: #4a90e2 !important; color: white !important; }
